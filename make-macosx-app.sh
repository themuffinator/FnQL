...
if [ ! -f Makefile ]; then
	echo "$0 must be run from the fnql build directory"
	exit 1
fi
...
FNQL_VERSION=`grep "^[[:space:]]*#define[[:space:]]\\+FNQL_VERSION_STRING[[:space:]]\\+\\\"" version/fnql_version.h | sed -e 's/.*"\\([^"]*\\)".*/\\1/'`
FNQL_CLIENT_ARCHS=""
FNQL_SERVER_ARCHS=""

BASEDIR="baseq3"

DEDICATED_NAME="fnql.ded"
...
for ARCH in $SEARCH_ARCHS; do
	CURRENT_ARCH=${ARCH}
	BUILT_PRODUCTS_DIR="${OBJROOT}/${TARGET_NAME}-darwin-${CURRENT_ARCH}"
	FNQL_CLIENT="${EXECUTABLE_NAME}.${CURRENT_ARCH}"
	FNQL_SERVER="${DEDICATED_NAME}.${CURRENT_ARCH}"
	...
	# executables
	if [ -e ${BUILT_PRODUCTS_DIR}/${FNQL_CLIENT} ]; then
		FNQL_CLIENT_ARCHS="${BUILT_PRODUCTS_DIR}/${FNQL_CLIENT} ${FNQL_CLIENT_ARCHS}"
		VALID_ARCHS="${ARCH} ${VALID_ARCHS}"
	else
		continue
	fi
	if [ -e ${BUILT_PRODUCTS_DIR}/${FNQL_SERVER} ]; then
		FNQL_SERVER_ARCHS="${BUILT_PRODUCTS_DIR}/${FNQL_SERVER} ${FNQL_SERVER_ARCHS}"
	fi

	#echo "valid arch: ${ARCH}"
done

if [ "${FNQL_CLIENT_ARCHS}" == "" ]; then
	echo "$0: no fnql binary architectures were found for target '${TARGET_NAME}'"
	exit 1
fi
...
<dict>
    ...
    <string>org.fnql.${PRODUCT_NAME}</string>
...
# executables
action "${BUNDLEBINDIR}/${EXECUTABLE_NAME}"				"${FNQL_CLIENT_ARCHS}"
action "${BUNDLEBINDIR}/${DEDICATED_NAME}"				"${FNQL_SERVER_ARCHS}"
