...
| Input | Additive QL mouse style with CPI normalization, signed power acceleration, cap, and bounded view-angle filtering; UTF-8/surrogate-safe character dispatch; opt-in QL WinMM joystick profile; preserved FnQ3 mouse styles and SDL3 gamepads; synchronized macOS Command-key modifier state | Retail executable constants and QLSRP owner mapping back the focused C++ fixtures. Native Intel and Apple-Silicon CI now compile and test the SDL path; windowed Apple-hardware input probes remain useful runtime evidence |
...
| Non-regression infrastructure | Focused C++/source contract tests, recursive Python discovery, live protocol/ZMQ scripts, private-cvar browser suppression, build-thread dependencies, corrected project-owned workflow identity, and native Intel/Apple-Silicon macOS build/bundle/signature/dependency gates | Current MSVC x86/x64 client/server builds and focused tests pass; windowed retail-module protocol/demo and live ZMQ probes pass. Retail-executable packet capture, notarized release credentials, and Apple-hardware renderer smoke remain broader promotion evidence |
...
| macOS | native x86_64/arm64 CI, SDL-only configure guards, canonical app/runtime paths, MoltenVK portability-subset enablement, deterministic unsupported-provider/module behavior, Mach-O packaging, signing/notarization tooling, and Apple-hardware windowed smoke evidence |
