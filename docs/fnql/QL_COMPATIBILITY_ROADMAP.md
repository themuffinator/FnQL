...
| Input | Additive QL mouse style with CPI normalization, signed power acceleration, cap, and bounded view-angle filtering; UTF-8/surrogate-safe character dispatch; opt-in QL WinMM joystick profile; preserved FnQ3 mouse styles and SDL3 gamepads; retained source-level macOS Command-key modifier handling | Retail executable constants and QLSRP owner mapping back the focused C++ fixtures. Focused source fixtures cover the retained SDL modifier path; macOS is not a current FnQL build or runtime target |
...
| macOS | Unsupported: build frontends must reject the available 64-bit Intel/Apple-Silicon targets; retained source paths receive static portability checks only and no package or retail-runtime claim |
