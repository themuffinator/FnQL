...
class CGameNativeBridgeSourceTests(unittest.TestCase):
    ...
    def test_native_getglconfig_uses_retail_ql_layout(self) -> None:
        ...
        self.assertIn("retailConfig.vidWidth = cls.captureWidth > 0 ? cls.captureWidth : cls.glconfig.vidWidth;", cl_cgame)
        self.assertIn("retailConfig.vidHeight = cls.captureHeight > 0 ? cls.captureHeight : cls.glconfig.vidHeight;", cl_cgame)
    ...
    def test_native_renderer_calls_bridge_public_and_private_supersample_geometry(self) -> None:
        client_h = read_repo_file("code/client/client.h")
        cl_cgame = read_repo_file("code/client/cl_cgame.cpp")
        imports = read_repo_file("code/client/ql_cgame_imports.inc")

        self.assertIn("} retailFramebufferScale_t;", client_h)
        self.assertIn("retailFramebufferScale_t CL_GetRetailFramebufferScale( void );", client_h)
        self.assertIn("scale.x = static_cast<float>( cls.glconfig.vidWidth ) / cls.captureWidth;", cl_cgame)
        self.assertIn("scale.y = static_cast<float>( cls.glconfig.vidHeight ) / cls.captureHeight;", cl_cgame)
        self.assertIn("CL_ScaleRetailRefdef( &scaledRefdef );", imports)
        self.assertIn("x *= scale.x;", imports)
        self.assertIn("y *= scale.y;", imports)
        self.assertIn("w *= scale.x;", imports)
        self.assertIn("h *= scale.y;", imports)
        self.assertNotIn("scaledRefdef.fov_x", imports)
        self.assertNotIn("scaledRefdef.fov_y", imports)
    ...
    def test_cgame_native_text_imports_use_shared_scaled_text_bridge(self) -> None:
        ...
        self.assertIn("scale * framebufferScale.y, limit, rendererMaxX", cl_cgame)
        self.assertIn("RE_MeasureScaledText( text, end, fontHandle, scale * framebufferScale.y, limit, bounds );", cl_cgame)
        self.assertIn("bounds[2] /= framebufferScale.x;", cl_cgame)
        self.assertIn("bounds[4] /= framebufferScale.y;", cl_cgame)
