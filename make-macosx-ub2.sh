...
cd `dirname $0`
if [ ! -f Makefile ]; then
	echo "This script must be run from the fnql build directory"
	exit 1
fi
...
if [ "$1" == "notarize" ]; then
	...
	# release build name
	RELEASE_BUILD="fnql.app"

	# Pre-notarized zip file (not what is shipped)
	PRE_NOTARIZED_ZIP="fnql_prenotarized.zip"

	# Post-notarized zip file (shipped)
	POST_NOTARIZED_ZIP="fnql_notarized.zip"

	BUNDLE_ID="org.fnql.fnql"
	...
	ENTITLEMENTS_FILE="misc/xcode/fnql/fnql.entitlements"
