...
is required. Official releases support the Windows x86 retail client and Linux
i686 dedicated server. The Windows package includes FnQL's open-source engine,
its WebUI overlay, and the compiled closed-source Steam provider. The Linux
package contains only redistributable FnQL components. No package includes
...
profile. Then download the supported Windows or Linux package from the
