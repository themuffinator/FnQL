...
public:
	...
	VidModeList( const VidModeList& ) = delete;
	VidModeList& operator=( const VidModeList& ) = delete;

	XF86VidModeModeInfo ***receive() noexcept
	{
		reset();
		return &modes_;
	}

	XF86VidModeModeInfo *operator[]( int index ) const noexcept
	{
		return modes_[ index ];
	}

	void reset( XF86VidModeModeInfo **modes = nullptr ) noexcept
	{
		if ( modes_ != nullptr && modes_ != modes )
		{
			std::free( modes_ );
		}
		modes_ = modes;
	...
	{
		return modes_ != nullptr;
	}

private:
	XF86VidModeModeInfo **modes_ = nullptr;
};

VidModeList vidmodes;

} // namespace

static GLW_ScopedLibrary v_lib;
...
qboolean VidMode_Init( void )
{
	...
	if ( !v_lib )
	{
		v_lib.reset( GLW_LoadLibraryFallback( "libXxf86vm.so.1", "libXxf86vm.so" ) );
		if ( !v_lib )
		{
	...
	if ( !GLW_LoadSymbols( v_lib.get(), v_list, "libXxf86vm" ) )
	{
		goto __fail;
	}
...
qboolean VidMode_SetMode( int *width, int *height, int *rate )
{
	...
	_XF86VidModeGetAllModeLines( dpy, scrnum, &num_vidmodes, vidmodes.receive() );
