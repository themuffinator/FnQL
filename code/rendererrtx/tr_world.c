...
static int R_CullAdvertisementQuad( const vec3_t points[4] ) {
	...
	for ( i = 0; i < 4; ++i ) {
		...
		for ( j = 0; j < 4; ++j ) {
			const float dist = DotProduct( points[j], frust->normal ) - frust->dist;
			if ( dist > 0.0f ) {
				front = 1;
				if ( back ) {
					break;
...
Retail QL advertisement brush models are submitted after the ordinary world
walk and remain raster-owned. This preserves their dynamic loading-view bridge
and keeps them out of the static RT world ownership decision.
=================
*/
void R_UpdateAdvertisements( void ) {
	...
	if ( !tr.world || tr.world->numAdvertisements <= 0 ||
		( tr.refdef.rdflags & ( RDF_NOWORLDMODEL | RDF_HYPERSPACE ) ) ) {
		return;
	}
	...
	for ( i = 0; i < tr.world->numAdvertisements; ++i ) {
		...
		if ( !advertisement->bmodel ||
			!R_inPVS( tr.refdef.vieworg, advertisement->center ) ) {
			continue;
		}
...
void R_AdvertisementList_f( void ) {
	...
	for ( i = 0; i < tr.world->numAdvertisements; ++i ) {
		...
		const bmodel_t *bmodel = advertisement->bmodel;
		const msurface_t *surface =
			( bmodel && bmodel->numSurfaces > 0 ) ? bmodel->firstSurface : NULL;
		const char *shaderName =
			( surface && surface->shader && surface->shader->name[0] ) ?
				surface->shader->name : "<null>";
		const int bmodelIndex = bmodel ? (int)( bmodel - tr.world->bmodels ) : -1;

		ri.Printf( PRINT_ALL,
			"advertlist: [%d] cellId=%d sourceIndex=%d model=*%d surfaces=%d shader=%s "
			"center=(%.1f %.1f %.1f) normal=(%.3f %.3f %.3f)\n",
			i, advertisement->cellId, advertisement->sourceIndex,
			bmodelIndex, bmodel ? bmodel->numSurfaces : 0, shaderName,
			advertisement->center[0], advertisement->center[1],
			advertisement->center[2], advertisement->normal[0],
			advertisement->normal[1], advertisement->normal[2] );
