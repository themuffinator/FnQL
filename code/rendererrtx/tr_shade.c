...
void VK_SetFogParams( vkUniform_t *uniformData, int *fogStage )
{
	if ( tess.fogNum && tess.shader->fogPass ) {
		...
		uniformData->fogEyeT[2] = r_fogMode && r_fogMode->integer ? 1.0f : 0.0f;
		// fragment data
...
void VK_CSMShadowPass( void )
{
	...
	vk_update_descriptor( VK_DESC_TEXTURE0, vk_csm_shadow_descriptor() );
...
void VK_LightingPass( void )
{
	...
	vk_update_descriptor( VK_DESC_TEXTURE2, shadowDescriptor );
...
void RB_EndSurface( void ) {
	...
	if ( tess.shader == tr.enemyRimShader ) {
		RB_EnemyRimTessEnd();
		/*
		 * The highlight handler owns this batch completely.  Clear it here so
		 * a defensive later RB_EndSurface cannot redraw it with stale entity
		 * transforms.
		 */
		tess.numIndexes = 0;
		tess.numVertexes = 0;
