...
static qboolean isStaticShader( shader_t *shader )
{
	...
	/* Enhanced liquids append synthetic refraction/reflection passes from the
	 * live tessellation data. Keep eligible surfaces off the static VBO path so
	 * their geometry and normals remain available to those passes. */
	if ( r_fbo && r_fbo->integer && r_liquid &&
		r_liquid->integer > 0 &&
