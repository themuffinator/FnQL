...
{
	return index < 4 ? liquid_amplitude[0][index] :
		liquid_amplitude[1][index - 4];
}

vec2 LiquidWaveGradient(vec3 position, float time)
{
	vec2 gradient = vec2(0.0);
	gradient += vec2(0.66, 0.75) *
		(0.42 * cos(dot(position, vec3(0.0096, 0.0109, 0.0051)) + time * 0.85));
	gradient += vec2(-0.81, 0.59) *
		(0.33 * cos(dot(position, vec3(-0.0269, 0.0196, 0.0116)) + time * 1.40));
	gradient += vec2(0.42, -0.91) *
		(0.25 * cos(dot(position, vec3(0.0320, -0.0693, 0.0266)) + time * 2.30));
	return gradient;
}
...
void main()
{
	...
	if (liquid_info.w > 0.5) {
		...
		for (int i = 0; i < impulse_count; ++i) {
			...
			float height_fade = 1.0 -
				clamp(abs(height) / max(48.0, width * 3.0), 0.0, 1.0);
			vec2 screen_gradient =
				vec2(dFdx(distance_to_center), dFdy(distance_to_center));
			float gradient_length = length(screen_gradient);
			vec2 direction = gradient_length > 0.0001 ?
				screen_gradient / gradient_length : vec2(0.0);
			ripple_pixels += direction * ring * height_fade *
				ImpulseAmplitude(i);
		}
		...
		vec2 sample_uv = clamp(
			uv + (ambient_pixels + ripple_pixels) * inverse_view * edge_fade,
			vec2(0.002), vec2(0.998));
		/* RTX composition uses reversed depth. A larger copied sample is
		 * foreground and must not be pulled through the waterline. */
		...
		vec2 reflected_uv =
			reflected_clip.xy / max(reflected_clip.w, 0.001) * 0.5 + 0.5;
		vec2 reflected_edge =
			smoothstep(vec2(0.0), vec2(0.08), reflected_uv)
		...
		vec3 sheen_color =
			mix(liquid_reflect.rgb, reflection_color, reflection_weight);
		float alpha = liquid_info.x * clamp(liquid_params.z, 0.0, 1.0)
