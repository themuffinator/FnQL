...
vec3 encodeHdr10Linear(vec3 linearSrgb) {
	vec3 bt2020 = max(linearSrgbToBt2020(max(linearSrgb, vec3(0.0))), vec3(0.0));
	return pqEncodeNits(bt2020 * hdrPaperWhiteNits);
}

vec3 encodeHdr10Srgb(vec3 color) {
	return encodeHdr10Linear(srgbToLinear(max(color, vec3(0.0))));
}
...
vec3 sampleColorGradeLut(vec3 color) {
	...
	vec2 uv0 = vec2((slice0 * lutSize + coord.r + 0.5) / atlasWidth,
		(coord.g + 0.5) / lutSize);
	vec2 uv1 = vec2((slice1 * lutSize + coord.r + 0.5) / atlasWidth, uv0.y);
	vec3 graded = mix(texture(colorGradeLut, uv0).rgb,
		texture(colorGradeLut, uv1).rgb, fracB);
