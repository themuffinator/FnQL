...
static constexpr int BOT_DEBUG_POLY_POINTS = 128;

struct bot_debugpoly_t {
	bool inuse;
	int color;
	int numPoints;
	vec3_t points[ BOT_DEBUG_POLY_POINTS ];
};

static bot_debugpoly_t *debugpolygons = nullptr;
static int bot_maxdebugpolys;

extern botlib_export_t	*botlib_export;
int	bot_enable;

static void BotImport_StoreDebugPolygon( bot_debugpoly_t &poly, int color, int numPoints, const vec3_t *points ) {
	numPoints = std::clamp( numPoints, 0, BOT_DEBUG_POLY_POINTS );
	...
	for ( int i = 0; i < numPoints; ++i ) {
		VectorCopy( points[i], poly.points[i] );
	}
}
...
int SV_BotAllocateClient( void ) {
	...
	// find a client slot
	for ( client_t &candidate : SV_Clients() ) {
...
void BotDrawDebugPolygons(void (*drawPoly)(int color, int numPoints, float *points), int value) {
	...
	if (bot_enable && bot_debug->integer) {
		...
		//
		const client_t &debugClient = SV_ClientForIndex( 0 );
		parm0 = 0;
		if (debugClient.lastUsercmd.buttons & BUTTON_ATTACK) parm0 |= 1;
		if (bot_reachability->integer) parm0 |= 2;
		if (bot_groundonly->integer) parm0 |= 4;
		botlib_export->BotLibVarSet("bot_highlightarea", bot_highlightarea->string);
		botlib_export->Test(parm0, nullptr, debugClient.gentity->r.currentOrigin, 
			debugClient.gentity->r.currentAngles);
	} //end if
	//draw all debug polys
	for (i = 0; i < bot_maxdebugpolys; i++) {
		poly = &debugpolygons[i];
		if (!poly->inuse) continue;
		drawPoly( poly->color, poly->numPoints, reinterpret_cast<float *>( poly->points ) );
...
static __attribute__ ((format (printf, 2, 3))) void QDECL BotImport_Print(int type, const char *fmt, ...)
{
	...
	va_start(ap, fmt);
	Q_vsnprintf( str.data(), SV_ArraySize(str), fmt, ap );
	va_end(ap);

	switch(type) {
		case PRT_MESSAGE: {
			Com_Printf("%s", str.data());
			break;
		}
		case PRT_WARNING: {
			Com_Printf(S_COLOR_WARNING "Warning: %s", str.data());
			break;
		}
		case PRT_ERROR: {
			Com_Printf(S_COLOR_ERROR "Error: %s", str.data());
			break;
		}
		case PRT_FATAL: {
			Com_Printf(S_COLOR_ERROR "Fatal: %s", str.data());
			break;
		}
		case PRT_EXIT: {
			Com_Error(ERR_DROP, S_COLOR_ERROR "Exit: %s", str.data());
			break;
		}
		default: {
...
int BotImport_DebugPolygonCreate(int color, int numPoints, vec3_t *points) {
	...
	if (i >= bot_maxdebugpolys)
		return 0;
	poly = &debugpolygons[i];
	BotImport_StoreDebugPolygon( *poly, color, numPoints, points );
	//
	return i;
}
...
static int BotImport_DebugLineCreate(void) {
	std::array<vec3_t, 1> points{};
	return BotImport_DebugPolygonCreate(0, 0, points.data());
}
...
static void BotImport_DebugLineShow(int line, vec3_t start, vec3_t end, int color) {
	std::array<vec3_t, 4> points{};
	vec3_t dir, cross, up = {0, 0, 1};
	float dot;
	...
	BotImport_DebugPolygonShow(line, color, SV_ArraySize(points), points.data());
}
...
static void BotClientCommand( int client, const char *command ) {
	if ( SV_IsClientIndex( client ) ) {
		SV_ExecuteClientCommand( &SV_ClientForIndex( client ), command );
	}
}
...
void SV_BotInitBotLib(void) {
	...
	SV_ZFree( debugpolygons );
	bot_maxdebugpolys = Cvar_VariableIntegerValue("bot_maxdebugpolys");
	debugpolygons = SV_ZMallocArray<bot_debugpoly_t>( bot_maxdebugpolys );
...
int EntityInPVS( int client, int entityNum ) {
	...
	for ( int i : SV_Indices( frame->num_entities ) )	{
...
int SV_BotGetSnapshotEntity( int client, int sequence ) {
	if ( SV_IsClientIndex( client ) ) {
		const client_t* cl = &SV_ClientForIndex( client );
