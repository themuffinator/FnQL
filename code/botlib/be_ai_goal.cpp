...
static constexpr int AVOID_MINIMUM_TIME = 10;
//default avoid goal time
static constexpr int AVOID_DEFAULT_TIME = 30;
//avoid dropped goal time
static constexpr int AVOID_DROPPED_TIME = 10;
//
static constexpr float TRAVELTIME_SCALE = 0.01f;
//item flags
static constexpr int IFL_NOTFREE = 1; //not in free for all
static constexpr int IFL_NOTTEAM = 2; //not in team play
static constexpr int IFL_NOTSINGLE = 4; //not in single player
static constexpr int IFL_NOTBOT = 8; //bot should never go for this
static constexpr int IFL_ROAM = 16; //bot roam goal
...
//item configuration
static itemconfig_t *itemconfig = nullptr;
//level items
static levelitem_t *levelitemheap = nullptr;
static levelitem_t *freelevelitems = nullptr;
static levelitem_t *levelitems = nullptr;
static int numlevelitems = 0;
//map locations
static maplocation_t *maplocations = nullptr;
//camp spots
static campspot_t *campspots = nullptr;
//the game type
static int g_gametype = 0;
//additional dropped item weight
static libvar_t *droppedweight = nullptr;
...
static itemconfig_t *LoadItemConfig( const char *filename )
{
	...
	ic = botlib::ClearedHunkAllocBytes<itemconfig_t>(sizeof(itemconfig_t) +
	...
	while(PC_ReadToken(source, &token))
	{
		if (!strcmp(token.string, "iteminfo"))
		{
			...
			*ii = {};
			...
			if (!ReadStructure(source, &iteminfo_struct, ii))
			{
				FreeMemory(ic);
				FreeSource(source);
				return nullptr;
...
static int *ItemWeightIndex(const weightconfig_t *iwc, const itemconfig_t *ic)
{
	...
	index = botlib::ClearedAlloc<int>(ic->numiteminfo);
...
static void InitLevelItemHeap(void)
{
	...
	levelitemheap = botlib::ClearedAlloc<levelitem_t>(max_levelitems);
...
static void BotInitInfoEntities(void)
{
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		{
			ml = botlib::ClearedAlloc<maplocation_t>();
		...
		{
			cs = botlib::ClearedAlloc<campspot_t>();
...
void BotResetAvoidGoals(int goalstate)
{
	...
	gs = BotGoalStateFromHandle(goalstate);
	if (!gs) return;
	std::fill_n(gs->avoidgoals, MAX_AVOIDGOALS, 0);
	std::fill_n(gs->avoidgoaltimes, MAX_AVOIDGOALS, 0.0f);
...
static constexpr int ET_ITEM = 2;
...
void BotResetGoalState(int goalstate)
{
	...
	gs = BotGoalStateFromHandle(goalstate);
	if (!gs) return;
	std::fill_n(gs->goalstack, MAX_GOALSTACK, bot_goal_t{});
	gs->goalstacktop = 0;
...
int BotAllocGoalState(int client)
{
	...
	for (i = 1; i <= MAX_CLIENTS; i++)
	{
		if (!botgoalstates[i])
		{
			botgoalstates[i] = botlib::ClearedAlloc<bot_goalstate_t>();
