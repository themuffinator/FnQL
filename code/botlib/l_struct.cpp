...
inline void *FieldAddress(void *structure, size_t offset)
{
	return static_cast<char *>(structure) + offset;
}

inline const void *FieldAddress(const void *structure, size_t offset)
{
	return static_cast<const char *>(structure) + offset;
}

inline void *AdvanceField(void *p, size_t size)
{
	return static_cast<char *>(p) + size;
}

inline const void *AdvanceField(const void *p, size_t size)
{
...
template <typename T>
const T &FieldValue(const void *p)
{
...
static qboolean ReadNumber(source_t *source, const fielddef_t *fd, void *p)
{
	...
	if (token.subtype & TT_FLOAT)
	{
		...
		FieldValue<float>(p) = static_cast<float>(floatval);
		return qtrue;
	} //end if
	//
	...
	if ((fd->type & FT_TYPE) == FT_CHAR)
	{
		if (fd->type & FT_UNSIGNED) FieldValue<unsigned char>(p) = static_cast<unsigned char>(intval);
		else FieldValue<char>(p) = static_cast<char>(intval);
	} //end if
	else if ((fd->type & FT_TYPE) == FT_INT)
	{
		if (fd->type & FT_UNSIGNED) FieldValue<unsigned int>(p) = static_cast<unsigned int>(intval);
		else FieldValue<int>(p) = static_cast<int>(intval);
	} //end else
	else if ((fd->type & FT_TYPE) == FT_FLOAT)
	{
		FieldValue<float>(p) = static_cast<float>(intval);
	} //end else
	return qtrue;
...
static qboolean ReadChar(source_t *source, const fielddef_t *fd, void *p)
{
	...
	if (token.type == TT_LITERAL)
	{
		...
		FieldValue<char>(p) = token.string[0];
	} //end if
	else
	{
...
static int ReadString(source_t *source, const fielddef_t *fd, void *p)
{
	...
	Q_strncpyz(static_cast<char *>(p), token.string, MAX_STRINGFIELD);
	return 1;
...
int ReadStructure(source_t *source, const structdef_t *def, void *structure)
{
	...
	while(1)
	{
		...
		p = FieldAddress(structure, fd->offset);
		while (num-- > 0)
		{
			if (fd->type & FT_ARRAY)
			{
			...
			switch(fd->type & FT_TYPE)
			{
				case FT_CHAR:
				{
					...
					p = AdvanceField(p, sizeof(char));
					break;
				} //end case
				case FT_INT:
				{
					if (!ReadNumber(source, fd, p)) return qfalse;
					p = AdvanceField(p, sizeof(int));
					break;
				} //end case
				case FT_FLOAT:
				{
					if (!ReadNumber(source, fd, p)) return qfalse;
					p = AdvanceField(p, sizeof(float));
					break;
				} //end case
				case FT_STRING:
				{
					if (!ReadString(source, fd, p)) return qfalse;
					p = AdvanceField(p, MAX_STRINGFIELD);
					break;
				} //end case
				case FT_STRUCT:
				{
					...
					if (!ReadStructure(source, fd->substruct, p)) return qfalse;
					p = AdvanceField(p, fd->substruct->size);
					break;
				} //end case
			} //end switch
			if (fd->type & FT_ARRAY)
...
// Changes Globals:		-
//===========================================================================
static int WriteStructWithIndent(FILE *fp, const structdef_t *def, const void *structure, int indent)
{
	int i, num;
	const void *p;
	const fielddef_t *fd;
	...
	for (i = 0; def->fields[i].name; i++)
	{
		...
		while(num-- > 0)
		{
			switch(fd->type & FT_TYPE)
			{
				case FT_CHAR:
				{
					if (fprintf(fp, "%d", FieldValue<char>(p)) < 0) return qfalse;
					p = AdvanceField(p, sizeof(char));
					break;
				} //end case
				case FT_INT:
				{
					if (fprintf(fp, "%d", FieldValue<int>(p)) < 0) return qfalse;
					p = AdvanceField(p, sizeof(int));
					break;
				} //end case
				case FT_FLOAT:
				{
					if (!WriteFloat(fp, FieldValue<float>(p))) return qfalse;
					p = AdvanceField(p, sizeof(float));
					break;
				} //end case
				case FT_STRING:
				{
					if (fprintf(fp, "\"%s\"", static_cast<const char *>(p)) < 0) return qfalse;
					p = AdvanceField(p, MAX_STRINGFIELD);
					break;
				} //end case
				case FT_STRUCT:
				{
					if (!WriteStructWithIndent(fp, fd->substruct, p, indent)) return qfalse;
					p = AdvanceField(p, fd->substruct->size);
					break;
				} //end case
			} //end switch
