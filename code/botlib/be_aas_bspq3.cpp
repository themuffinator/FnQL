...
[[maybe_unused]] static constexpr float ON_EPSILON = 0.005f;
//#define DEG2RAD( a ) (( a * M_PI ) / 180.0F)

static constexpr int MAX_BSPENTITIES = 2048;
#if 0
struct rgb_s
{
	int red;
...
static void AAS_ParseBSPEntities(void)
{
	...
	while(PS_ReadToken(script, &token))
	{
		...
		while(PS_ReadToken(script, &token))
		{
			...
			epair = botlib::ClearedHunkAlloc<bsp_epair_t>();
			...
			epair->key = botlib::HunkCopyString(token.string);
			...
			StripDoubleQuotes(token.string);
			epair->value = botlib::HunkCopyString(token.string);
		} //end while
		if (strcmp(token.string, "}"))
		{
...
int AAS_LoadBSPFile(void)
{
	...
	bspworld.dentdata = botlib::ClearedHunkAlloc<char>(bspworld.entdatasize);
