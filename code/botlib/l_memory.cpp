...
static constexpr unsigned long int MEM_ID = 0x12345678ul;
static constexpr unsigned long int HUNK_ID = 0x87654321ul;
...
#endif //MEMDEBUG
{
	...
	block = static_cast<memoryblock_t *>(ptr);
	block->id = MEM_ID;
	block->ptr = static_cast<char *>(ptr) + sizeof(memoryblock_t);
...
static memoryblock_t *BlockFromPointer(void *ptr, const char *str)
{
	...
	{
#ifdef MEMDEBUG
		//char *crash = static_cast<char *>(nullptr);
		//crash[0] = 1;
		botimport.Print(PRT_FATAL, "%s: nullptr pointer\n", str);
#endif // MEMDEBUG
		return nullptr;
	} //end if
	block = reinterpret_cast<memoryblock_t *>(static_cast<char *>(ptr) - sizeof(memoryblock_t));
...
#endif //MEMDEBUG
{
	...
	memid = static_cast<unsigned long int *>(ptr);
	*memid = MEM_ID;
	return static_cast<char *>(ptr) + sizeof(unsigned long int);
...
void FreeMemory(void *ptr)
{
	...
	memid = reinterpret_cast<unsigned long int *>(static_cast<char *>(ptr) - sizeof(unsigned long int));
