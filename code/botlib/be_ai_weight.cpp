...
static constexpr int MAX_INVENTORYVALUE = 999999;
#define EVALUATERECURSIVELY

static constexpr int MAX_WEIGHT_FILES = 128;
...
static fuzzyseperator_t *ReadFuzzySeperators_r(source_t *source)
{
	...
	do
	{
		...
		{
			fs = botlib::ClearedAlloc<fuzzyseperator_t>();
			fs->index = index;
...
weightconfig_t *ReadWeightConfig(const char *filename)
{
	...
	//
	config = botlib::ClearedAlloc<weightconfig_t>();
	config->numweights = 0;
	...
	while(PC_ReadToken(source, &token))
	{
		if (!strcmp(token.string, "weight"))
		{
			...
			config->weights[config->numweights].name = botlib::ClearedAlloc<char>(strlen(token.string) + 1);
