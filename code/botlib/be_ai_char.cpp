...
static constexpr int MAX_CHARACTERISTICS = 80;

static constexpr char CT_INTEGER = 1;
static constexpr char CT_FLOAT = 2;
static constexpr char CT_STRING = 3;

static constexpr const char *DEFAULT_CHARACTER = "bots/default_c.c";
...
static constexpr int MAX_HANDLES = MAX_CLIENTS * 2;
...
static void BotDefaultCharacteristics(bot_character_t *ch, bot_character_t *defaultch)
{
	...
	for (i = 0; i < MAX_CHARACTERISTICS; i++)
	{
		...
		else if (defaultch->c[i].type == CT_STRING)
		{
			...
			ch->c[i].value.string = botlib::CopyString(defaultch->c[i].value.string);
		} //end else if
	} //end for
...
static bot_character_t *BotLoadCharacterFromFile(const char *charfile, int skill)
{
	...
	ch = botlib::ClearedAlloc<bot_character_t>();
			...
			if (skill < 0 || (int)token.intvalue == skill)
			{
				...
				while(PC_ExpectAnyToken(source, &token))
				{
					...
					else if (token.type == TT_STRING)
					{
						...
						ch->c[index].value.string = botlib::CopyString(token.string);
...
static int BotInterpolateCharacters(int handle1, int handle2, float desiredskill)
{
	...
	out = botlib::ClearedAlloc<bot_character_t>();
	...
	for (i = 0; i < MAX_CHARACTERISTICS; i++)
	{
		...
		else if (ch1->c[i].type == CT_STRING)
		{
			...
			out->c[i].value.string = botlib::CopyString(ch1->c[i].value.string);
		} //end else if
	} //end for
	return handle;
