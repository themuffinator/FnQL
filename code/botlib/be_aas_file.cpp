...
// Changes Globals:		-
//===========================================================================
static byte *AAS_LoadAASLumpBytes(fileHandle_t fp, int offset, int length, int *lastoffset, size_t elementSize)
{
	byte *buf;
	//
	if (!length)
	{
		//just alloc a dummy
		return botlib::ClearedHunkAlloc<byte>(elementSize + 1);
	} //end if
	//seek to the data
	...
	//allocate memory
	buf = botlib::ClearedHunkAlloc<byte>(length + 1);
	//read the data
	//if (length)
	{
	...
	return buf;
} //end of the function AAS_LoadAASLumpBytes

template <typename T>
static T *AAS_LoadAASLump(fileHandle_t fp, int offset, int length, int *lastoffset)
{
	return reinterpret_cast<T *>(AAS_LoadAASLumpBytes(fp, offset, length, lastoffset, sizeof(T)));
...
// Changes Globals:		-
//===========================================================================
static void AAS_DData(byte *data, int size)
{
	int i;

	for (i = 0; i < size; i++)
	{
		data[i] ^= static_cast<byte>(i * 119);
	} //end for
...
int AAS_LoadAASFile(const char *filename)
{
	...
	{
		AAS_DData(reinterpret_cast<byte *>(&header) + 8, sizeof(aas_header_t) - 8);
	} //end if
	//
	...
	aasworld.bboxes = AAS_LoadAASLump<aas_bbox_t>(fp, offset, length, &lastoffset);
	...
	aasworld.vertexes = AAS_LoadAASLump<aas_vertex_t>(fp, offset, length, &lastoffset);
	...
	aasworld.planes = AAS_LoadAASLump<aas_plane_t>(fp, offset, length, &lastoffset);
	...
	aasworld.edges = AAS_LoadAASLump<aas_edge_t>(fp, offset, length, &lastoffset);
	...
	aasworld.edgeindex = AAS_LoadAASLump<aas_edgeindex_t>(fp, offset, length, &lastoffset);
	...
	aasworld.faces = AAS_LoadAASLump<aas_face_t>(fp, offset, length, &lastoffset);
	...
	aasworld.faceindex = AAS_LoadAASLump<aas_faceindex_t>(fp, offset, length, &lastoffset);
	...
	aasworld.areas = AAS_LoadAASLump<aas_area_t>(fp, offset, length, &lastoffset);
	...
	aasworld.areasettings = AAS_LoadAASLump<aas_areasettings_t>(fp, offset, length, &lastoffset);
	...
	aasworld.reachability = AAS_LoadAASLump<aas_reachability_t>(fp, offset, length, &lastoffset);
	...
	aasworld.nodes = AAS_LoadAASLump<aas_node_t>(fp, offset, length, &lastoffset);
	...
	aasworld.portals = AAS_LoadAASLump<aas_portal_t>(fp, offset, length, &lastoffset);
	...
	aasworld.portalindex = AAS_LoadAASLump<aas_portalindex_t>(fp, offset, length, &lastoffset);
	...
	aasworld.clusters = AAS_LoadAASLump<aas_cluster_t>(fp, offset, length, &lastoffset);
...
static int AAS_WriteAASLump(fileHandle_t fp, aas_header_t *h, int lumpnum, const void *data, int length)
{
	aas_lump_t *lump;
...
template <typename T>
static int AAS_WriteAASLumpArray(fileHandle_t fp, aas_header_t *h, int lumpnum, const T *data, int count)
{
	assert(count >= 0);
	const size_t length = sizeof(T) * static_cast<size_t>(count);
	assert(length <= static_cast<size_t>(INT_MAX));
	return AAS_WriteAASLump(fp, h, lumpnum, data, static_cast<int>(length));
} //end of the function AAS_WriteAASLumpArray
//===========================================================================
...
// Changes Globals:		-
//===========================================================================
qboolean AAS_WriteAASFile(const char *filename)
{
	aas_header_t header = {};
	fileHandle_t fp;
	...
	if (!AAS_WriteAASLumpArray(fp, &header, AASLUMP_BBOXES, aasworld.bboxes,
		aasworld.numbboxes)) return qfalse;
	if (!AAS_WriteAASLumpArray(fp, &header, AASLUMP_VERTEXES, aasworld.vertexes,
		aasworld.numvertexes)) return qfalse;
	if (!AAS_WriteAASLumpArray(fp, &header, AASLUMP_PLANES, aasworld.planes,
		aasworld.numplanes)) return qfalse;
	if (!AAS_WriteAASLumpArray(fp, &header, AASLUMP_EDGES, aasworld.edges,
		aasworld.numedges)) return qfalse;
	if (!AAS_WriteAASLumpArray(fp, &header, AASLUMP_EDGEINDEX, aasworld.edgeindex,
		aasworld.edgeindexsize)) return qfalse;
	if (!AAS_WriteAASLumpArray(fp, &header, AASLUMP_FACES, aasworld.faces,
		aasworld.numfaces)) return qfalse;
	if (!AAS_WriteAASLumpArray(fp, &header, AASLUMP_FACEINDEX, aasworld.faceindex,
		aasworld.faceindexsize)) return qfalse;
	if (!AAS_WriteAASLumpArray(fp, &header, AASLUMP_AREAS, aasworld.areas,
		aasworld.numareas)) return qfalse;
	if (!AAS_WriteAASLumpArray(fp, &header, AASLUMP_AREASETTINGS, aasworld.areasettings,
		aasworld.numareasettings)) return qfalse;
	if (!AAS_WriteAASLumpArray(fp, &header, AASLUMP_REACHABILITY, aasworld.reachability,
		aasworld.reachabilitysize)) return qfalse;
	if (!AAS_WriteAASLumpArray(fp, &header, AASLUMP_NODES, aasworld.nodes,
		aasworld.numnodes)) return qfalse;
	if (!AAS_WriteAASLumpArray(fp, &header, AASLUMP_PORTALS, aasworld.portals,
		aasworld.numportals)) return qfalse;
	if (!AAS_WriteAASLumpArray(fp, &header, AASLUMP_PORTALINDEX, aasworld.portalindex,
		aasworld.portalindexsize)) return qfalse;
	if (!AAS_WriteAASLumpArray(fp, &header, AASLUMP_CLUSTERS, aasworld.clusters,
		aasworld.numclusters)) return qfalse;
