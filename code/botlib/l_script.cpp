...
static void PS_CreatePunctuationTable(script_t *script, punctuation_t *punctuations)
{
	...
	if (!script->punctuationtable) script->punctuationtable = botlib::Alloc<punctuation_t *>(256);
	std::fill_n(script->punctuationtable, 256, nullptr);
...
// Changes Globals:		-
//============================================================================
static const char *PS_TokenTypeName(int type)
{
	switch (type)
	{
...
} //end of the function PS_TokenTypeName

static void PS_NumberSubtypeName(int subtype, char *str, size_t size)
{
	str[0] = '\0';
...
} //end of the function PS_NumberSubtypeName
...
int PS_ExpectTokenType(script_t *script, int type, int subtype, token_t *token)
{
	...
	if (token->type != type)
	{
		ScriptError(script, "expected a %s, found %s", PS_TokenTypeName(type), token->string);
		return 0;
	} //end if
	if (token->type == TT_NUMBER)
	{
		...
		{
			PS_NumberSubtypeName(subtype, str, sizeof(str));
...
script_t *LoadScriptFile(const char *filename)
{
	...
	script = botlib::ClearedAllocBytes<script_t>(sizeof(script_t) + length + 1);
	*script = {};
	Q_strncpyz(script->filename, filename, sizeof(script->filename));
	script->buffer = reinterpret_cast<char *>(script) + sizeof(script_t);
