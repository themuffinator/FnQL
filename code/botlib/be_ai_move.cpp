...
#define AVOIDREACH
static constexpr int AVOIDREACH_TIME = 6; //avoid links for 6 seconds after use
static constexpr int AVOIDREACH_TRIES = 4;
//prediction times
static constexpr int PREDICTIONTIME_JUMP = 3; //in seconds
[[maybe_unused]] static constexpr int PREDICTIONTIME_MOVE = 2; //in seconds
//weapon indexes for weapon jumping
[[maybe_unused]] static constexpr int WEAPONINDEX_ROCKET_LAUNCHER = 5;
[[maybe_unused]] static constexpr int WEAPONINDEX_BFG = 9;

static constexpr int MODELTYPE_FUNC_PLAT = 1;
static constexpr int MODELTYPE_FUNC_BOB = 2;
static constexpr int MODELTYPE_FUNC_DOOR = 3;
static constexpr int MODELTYPE_FUNC_STATIC = 4;
...
int BotAllocMoveState(void)
{
	...
	for (i = 1; i <= MAX_CLIENTS; i++)
	{
		if (!botmovestates[i])
		{
			botmovestates[i] = botlib::ClearedAlloc<bot_movestate_t>();
			return i;
		} //end if
	} //end for
	return 0;
...
void BotSetBrushModelTypes(void)
{
	...
	std::fill_n(modeltypes, MAX_MODELS, 0);
	//
...
void BotResetAvoidReach(int movestate)
{
	...
	ms = BotMoveStateFromHandle(movestate);
	if (!ms) return;
	std::fill_n(ms->avoidreach, MAX_AVOIDREACH, 0);
	std::fill_n(ms->avoidreachtimes, MAX_AVOIDREACH, 0.0f);
	std::fill_n(ms->avoidreachtries, MAX_AVOIDREACH, 0);
