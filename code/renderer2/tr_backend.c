...
static const void *RB_DrawAdvertisementQueries( const void *data ) {
	...
	if ( !glRefConfig.occlusionQuery || !qglBeginQuery || !qglEndQuery || cmd->numEntries <= 0 ) {
		return (const void *)(cmd + 1);
	}
	...
	qglGetBooleanv( GL_COLOR_WRITEMASK, colorMask );
	...
	for ( i = 0 ; i < cmd->numEntries ; i++ ) {
		qglDepthFunc( GL_EQUAL );
		qglBeginQuery( GL_SAMPLES_PASSED, cmd->entries[i].occlusionQueryIds[0] );
		RB_DrawAdvertisementQueryQuad( cmd->entries[i].points );
		qglEndQuery( GL_SAMPLES_PASSED );

		qglDepthFunc( GL_LEQUAL );
		qglBeginQuery( GL_SAMPLES_PASSED, cmd->entries[i].occlusionQueryIds[1] );
		RB_DrawAdvertisementQueryQuad( cmd->entries[i].points );
		qglEndQuery( GL_SAMPLES_PASSED );
	}

	GL_Cull( cullType );
	qglColorMask( colorMask[0], colorMask[1], colorMask[2], colorMask[3] );
	if ( blendEnabled ) {
		qglEnable( GL_BLEND );
	}
	if ( depthMaskEnabled ) {
...
static const void *RB_PostProcess(const void *data)
{
	...
	{
		ivec4_t debugBox;
		VectorSet4(debugBox, 0, glConfig.vidHeight - 128, 128, 128);
		FBO_BlitFromTexture(tr.sunShadowDepthImage[0], NULL, NULL, NULL, debugBox, NULL, NULL, 0);
		VectorSet4(debugBox, 128, glConfig.vidHeight - 128, 128, 128);
		FBO_BlitFromTexture(tr.sunShadowDepthImage[1], NULL, NULL, NULL, debugBox, NULL, NULL, 0);
		VectorSet4(debugBox, 256, glConfig.vidHeight - 128, 128, 128);
		FBO_BlitFromTexture(tr.sunShadowDepthImage[2], NULL, NULL, NULL, debugBox, NULL, NULL, 0);
		VectorSet4(debugBox, 384, glConfig.vidHeight - 128, 128, 128);
		FBO_BlitFromTexture(tr.sunShadowDepthImage[3], NULL, NULL, NULL, debugBox, NULL, NULL, 0);
	}

	if (0 && r_shadows->integer == 4)
	{
		ivec4_t debugBox;
		VectorSet4(debugBox, 512 + 0, glConfig.vidHeight - 128, 128, 128);
		FBO_BlitFromTexture(tr.pshadowMaps[0], NULL, NULL, NULL, debugBox, NULL, NULL, 0);
		VectorSet4(debugBox, 512 + 128, glConfig.vidHeight - 128, 128, 128);
		FBO_BlitFromTexture(tr.pshadowMaps[1], NULL, NULL, NULL, debugBox, NULL, NULL, 0);
		VectorSet4(debugBox, 512 + 256, glConfig.vidHeight - 128, 128, 128);
		FBO_BlitFromTexture(tr.pshadowMaps[2], NULL, NULL, NULL, debugBox, NULL, NULL, 0);
		VectorSet4(debugBox, 512 + 384, glConfig.vidHeight - 128, 128, 128);
		FBO_BlitFromTexture(tr.pshadowMaps[3], NULL, NULL, NULL, debugBox, NULL, NULL, 0);
	}

	if (0)
	{
		ivec4_t debugBox;
		VectorSet4(debugBox, 256, glConfig.vidHeight - 256, 256, 256);
		FBO_BlitFromTexture(tr.renderDepthImage, NULL, NULL, NULL, debugBox, NULL, NULL, 0);
		VectorSet4(debugBox, 512, glConfig.vidHeight - 256, 256, 256);
		FBO_BlitFromTexture(tr.screenShadowImage, NULL, NULL, NULL, debugBox, NULL, NULL, 0);
	}
	...
	if (0)
	{
		ivec4_t debugBox;
		VectorSet4(debugBox, 256, glConfig.vidHeight - 256, 256, 256);
		FBO_BlitFromTexture(tr.sunRaysImage, NULL, NULL, NULL, debugBox, NULL, NULL, 0);
	}
